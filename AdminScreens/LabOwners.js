...
class LabOwners extends Component {
  constructor(props) {
    super(props);
    this.state = {
      doctors :[],
  ...
  getDoctors = async()=>{
    ...
    let api = `${url}/api/profile/userss/?position=LabAssistant&limit=10&offset=${this.state.offset}`
  ...
  searchDoctor =async(query)=>{
    let api  = `${url}/api/profile/userss/?search=${query}&role=LabAssistant`
         ...
         {!this.state.search ?  <View style={{ height: height * 0.1, backgroundColor: themeColor, borderBottomRightRadius: 20, borderBottomLeftRadius: 20, flexDirection: 'row', alignItems: "center" }}>
          ...
          <Ionicons name="chevron-back-circle" size={30} color="#fff" />
                ...
                <View style={{ flex: 0.6, alignItems: "center", justifyContent: "center" }}>
                  <Text style={[styles.text, { color: '#fff', fontWeight: 'bold', fontSize: 18 }]}>Lab Owners</Text>
                </View>
             <TouchableOpacity style={{ flex: 0.2, alignItems: "center", justifyContent: 'center' }}
          ...
          { <View style={{
                            ...
                            <TouchableOpacity
                                onPress={() => { this.props.navigation.navigate('CreateDiagnosticCenterOwner') }}
                            >
...
export default connect(mapStateToProps, { selectTheme })(LabOwners);